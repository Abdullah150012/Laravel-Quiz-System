        ...
        <div class="bg-white p-8 rounded-2xl shadow-lg w-full max-w-sm">
            @if (!Session('quizDetails'))
                ...
                <form action="add-quiz" method="get" class="space-y-4">
                    {{-- @csrf --}}
                    <div>
                        <input type="text" name="quiz" placeholder="Add Quiz" class="w-full px-3 py-2 border border-gray-500 rounded-xl focus:outline-none" required>
                    </div>
                    <div>
            ...
            @else
                <span class="text-xl font-bold text-green-500">{{Session('quizDetails')->name}}</span>
                <p class="text-xl font-bold text-green-500">Total MCQS : {{$totalMCQs}}
                    @if($totalMCQs > 0)
                        <a class="text-sm text-blue-500" href="/show-quiz/{{Session('quizDetails')->id}}" target="_blank">Show MCQs</a>
                    @endif
                </p>
                ...
                <form action="add-mcqs" method="post" class="space-y-4">
                    ...
                    <a href="/end-quiz" class="w-full block text-center bg-red-500  p-2 text-white rounded-xl cursor-pointer">Finish and Leave</a>
                </form>
            @endif
    </div>
    </div>
